...
Нам нужна еще одна матрица. При проектировании на прямую в знаменателе стояло число $a^\mathrm{T} a$. При проектировании на подпространство это число превращается в матрицу $A^\mathrm{T} A$, ранг которой может быть вычислен заранее.

**3F.** *Для произвольной матрицы $A$ размера $m \times n$, имеющей ранг $r$, произведение $A^\mathrm{T} A$ является симметрической матрицей ранга $r$.*

Вспомним, что *симметрической называется матрица, совпадающая со своей транспонированной*. Поэтому вычислим транспонированную к $A^\mathrm{T} A$ матрицу по правилу (9):
$$(A^\mathrm{T} A)^\mathrm{T} = A^\mathrm{T} (A^\mathrm{T})^\mathrm{T}. \qquad (10)$$
Но при повторном транспонировании мы вновь вернемся к исходной матрице $A$. Поэтому правая часть в (10) совпадает с $A^\mathrm{T} A$ и (10) показывает, что матрица совпадает со своей транспонированной. Другими словами, матрица $A^\mathrm{T} A$ является симметрической.
Для отыскания ее ранга мы покажем, что матрицы $A$ и $A^\mathrm{T} A$ имеют одинаковые нуль-пространства. Тогда, поскольку ранг плюс размерность нуль-пространства равняется числу столбцов матрицы, $r + (n - r) = n$, и матрицы $A$ и $A^\mathrm{T} A$ имеют по $n$ столбцов, отсюда немедленно вытекает равенство рангов матриц. Если вектор $x$ принадлежит нуль-пространству матрицы $A$, то $Ax = 0$ и $A^\mathrm{T} Ax = A^\mathrm{T} 0 = 0$, так что $x$ принадлежит и нуль-пространству матрицы $A^\mathrm{T} A$. Обратно, пусть $A^\mathrm{T} Ax = 0$. Умножая скалярно на вектор $x$, получаем
$$x^\mathrm{T} A^\mathrm{T} Ax = 0, \quad \text{или} \quad \| Ax \|^2 = 0, \quad \text{или} \quad Ax = 0.$$
Следовательно, $x$ принадлежит нуль-пространству матрицы $A$ и оба нуль-пространства совпадают.

Отметим один случай, который является как наиболее важным, так и наиболее распространенным. Это случай, когда столбцы матрицы $A$ линейно независимы, так что ее ранг $r$ равен $n$. Тогда, согласно **3F**, матрица $A^\mathrm{T} A$ размера $n \times n$ также будет иметь ранг, равный $n$. Следовательно, эта матрица обратима.

**3G.** *Если столбцы матрицы $A$ линейно независимы, так что $r = n$, то $A^\mathrm{T} A$ является квадратной симметрической обратимой матрицей.*

Легко привести пример такой матрицы:
$$A = \begin{bmatrix} 1 & 4 \\ 2 & 0 \\ 3 & 1 \end{bmatrix}, \qquad A^\mathrm{T} A = \begin{bmatrix} 14 & 7 \\ 7 & 17 \end{bmatrix}.$$

---
**стр. 133**
---

Обе матрицы имеют линейно независимые столбцы, ранг равен 2, и нуль-пространство отсутствует.

**Упражнение 3.1.1.** (a) Пусть заданы два произвольных положительных числа $x$ и $y$. Рассмотрим векторы $b = (\sqrt{x}, \sqrt{y})$ и $a = (\sqrt{y}, \sqrt{x})$. Используя неравенство Шварца, сравнить среднее арифметическое чисел $x$ и $y$ с их средним геометрическим.
(b) Пусть задан вектор с началом в нуле и концом в точке $x$ и к нему добавлен вектор длины $\|y\|$, соединяющий точку $x$ с $x+y$. Третья сторона треугольника образуется отрезком, соединяющим начало координат с этой точкой $x+y$. Тогда *неравенство треугольника утверждает, что это расстояние не может превосходить сумму первых двух*:
$$\| x + y \| \leqslant \| x \| + \| y \|.$$
Возводя обе части в квадрат, привести это к неравенству Шварца.

**Упражнение 3.1.2.** Проверить справедливость теоремы Пифагора для треугольника $Obp$, изображенного на рис. 3.3, используя формулу (5) для длины стороны $bp$.

**Упражнение 3.1.3.** Найти на луче, проходящем через начало координат и через точку $a=(1, 1, 1)$, точку $p$, ближайшую к точке $b=(2, 4, 4)$. Найти также точку, ближайшую к $a$, на луче, проходящем через начало координат и точку $b$.

**Упражнение 3.1.4.** Объяснить, почему неравенство Шварца превращается в равенство, когда точки $a$ и $b$ лежат на одной прямой, проходящей через начало координат, и только в этом случае. Что произойдет, если они будут расположены по разные стороны от начала координат?

**Упражнение 3.1.5.** Какой угол образует вектор $(1, 1, \dots, 1)$ с осями координат в $n$-мерном пространстве?

**Упражнение 3.1.6.** Неравенство Шварца доказывается в одну строчку, если векторы $a$ и $b$ заранее нормированы к единичной длине:
$$| a^\mathrm{T} b | = \left| \sum a_j b_j \right| \leqslant \sum | a_j | | b_j | \leqslant \sum \frac{| a_j |^2 + | b_j |^2}{2} = \frac{1}{2} + \frac{1}{2} = \| a \| \| b \|.$$
Провести доказательство справедливости второго неравенства.

**Упражнение 3.1.7.** Транспонирование матрицы $AA^{-1} = I$ показывает, что операции транспонирования и обращения перестановочны: $(A^{-1})^\mathrm{T} = (A^\mathrm{T})^{-1}$. Показать, что если матрица $A$ является симметрической, то симметрической будет и $A^{-1}$.

**Упражнение 3.1.8.** Построить симметрические матрицы $A$ и $B$ размера $2 \times 2$, произведение $AB$ которых не является симметрической матрицей. Заметим, что если матрицы $A$ и $B$ коммутируют, то их произведение будет симметрической матрицей: $(AB)^\mathrm{T} = B^\mathrm{T} A^\mathrm{T} = BA = AB$.

**Упражнение 3.1.9.** Если матрица $A$ имеет ранг $r$, то тот же ранг имеют и матрицы $A^\mathrm{T}$ (ранг по строкам равен рангу по столбцам), $A^\mathrm{T} A$ (согласно **3F**) и $AA^\mathrm{T}$ (согласно **3F**, примененному к матрице $A^\mathrm{T}$). Показать, что матрица $AA^\mathrm{T}$ может не быть обратимой, даже если $A^\mathrm{T} A$ является таковой.
